...
const StepView: React.FC<StepViewProps> = ({ step }) => {
  ...
  const handleStart = () => startStep(step.id);
  ...
  return (
    <div 
        className="flex flex-col h-full animate-fadeIn max-w-md mx-auto w-full relative"
          ...
          {/* Duration Display under Circle */}
           {!isTimerRunning && !isCompleted && step.durationMin > 0 && (
               <div className="mt-4 flex flex-col items-center">
                   <span className="font-mono text-2xl text-charcoal">{step.durationMin}</span>
                   <span className="text-[10px] text-stone-400 uppercase tracking-widest">דקות</span>
               </div>
           )}
        </div>
