...
const GlobalTimer: React.FC = () => {
    ...
    return (
        <button 
            ...
            className="absolute left-1/2 -translate-x-1/2 top-4 bg-charcoal text-white pl-3 pr-4 py-1.5 rounded-full flex items-center gap-2 shadow-lg hover:scale-105 transition-transform animate-slideUp z-50"
        >
            <div className="w-1.5 h-1.5 rounded-full bg-crust animate-pulse" />
            <span className="font-mono text-sm tracking-tight">{formatTime(timeLeft)}</span>
            <span className="text-[10px] opacity-60 border-l border-white/20 pl-2 ml-1">חזור לשלב</span>
        </button>
    );
};
